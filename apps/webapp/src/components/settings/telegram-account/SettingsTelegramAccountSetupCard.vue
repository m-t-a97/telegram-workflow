...
<template>
  <div>
    <va-card>
      <va-card-title class="text-base font-bold text-black">Telegram Account</va-card-title>
      <va-card-content class="flex flex-row justify-between items-center">
        <span class="text-base font-bold" :class="{
          'text-green-500': isConnectedToTelegram,
          'text-red-500': !isConnectedToTelegram,
        }">{{ isConnectedToTelegram ? "Connected" : "Not Connected" }}</span>

        <va-button v-if="!isConnectedToTelegram" class="flex-[0.35]" outline color="#10B981" :rounded="false"
          :loading="reconnectingToTelegram" :disabled="reconnectingToTelegram"
          @click="onOpenTelegramAccountSetupModal()">CONNECT</va-button>
      </va-card-content>
    </va-card>

    <SettingsTelegramAccountSetupCardConnectModal :showTelegramAccountSetupModal="showTelegramAccountSetupModal"
